...
from SUJAY.util.keepalive import ping_server
from SUJAY.bot.clients import initialize_clients
...
async def start():
    ...
    bot_info = await SUJAYBot.get_me()
    ...
    for name in files:
        with open(name) as a:
            ...
            print("SUJAY Imported => " + plugin_name)
    if ON_HEROKU:
    ...
    await SUJAYBot.send_message(chat_id=LOG_CHANNEL, text=script.RESTART_TXT.format(today, time))
...
if __name__ == '__main__':
    try:
        ...
        logging.info('Service Stopped Bye 😊❤️')
